...
public class Evaluator {
	...
	public static void main(String[] args) {
		...
		String[] rType = new String[]{"MART", "RankNet", "RankBoost", "AdaRank", "Coordinate Ascent", "LambdaRank", "LambdaMART", "ListNet", "Random Forests", "Linear Regression", "Ensemble"};
		RANKER_TYPE[] rType2 = new RANKER_TYPE[]{RANKER_TYPE.MART, RANKER_TYPE.RANKNET, RANKER_TYPE.RANKBOOST, RANKER_TYPE.ADARANK, RANKER_TYPE.COOR_ASCENT,
			 RANKER_TYPE.LAMBDARANK, RANKER_TYPE.LAMBDAMART, RANKER_TYPE.LISTNET, RANKER_TYPE.RANDOM_FOREST, RANKER_TYPE.LINEAR_REGRESSION, RANKER_TYPE.ENSEMBLE};
		...
		if(args.length < 2)
		{
			...
			System.out.println("\t\t\t\t10: Ensemble Method");
			...
			System.out.println("");
			System.out.println("    [-] Ensemble Method-specific parameters");
			System.out.println("\t[ -rankers <r> ]\t\t Comma separated rankers to use (default=" + "" + ")");
	...
	public RankListIterable readInput(String inputFile, int chunkSize) {
		return new RankListIterable(inputFile, chunkSize, mustHaveRelDoc, useSparseRepresentation);
	}
	...
	{
		RankListIterable samplesIterable = readInput(trainFile, 100000);

		int[] features = null;
		Ranker ranker = null;
		RankerTrainer trainer = new RankerTrainer();

		for(List<RankList> samples : samplesIterable) {
			if(features == null)
				features = FeatureManager.getFeatureFromSampleVector(samples);

			ranker = trainer.train(type, samples, null, features, trainScorer);
			...
			if(modelFile.compareTo("")!=0)
			{
				...
				AdaRank.state = modelFile;
				System.out.println("State saved to: " + modelFile);
			}
		}
		RankListIterable testIterable = readInput(testFile, 100000);
		double rankScore = 0.0;
		int count = 0;
		for(List<RankList> testSamples : testIterable) {
			if (normalize)
				normalize(testSamples, features);

			rankScore += evaluate(ranker, testSamples);
			count++;
		}
		System.out.println(testScorer.name() + " on test data: " + SimpleMath.round(rankScore/count, 4));
	}
	...
	{
		RankListIterable samplesIterable = readInput(trainFile, 50000);
		...
		RankListIterable testIterable = readInput(testFile, 50000);
		double rankScore = 0.0;
		int count = 0;
		for(List<RankList> testSamples : testIterable) {
			if (normalize)
...
class RankListIterable implements  Iterable<List<RankList>> {

	private String input;
	private int chunk;
	private boolean mustHaveRelDoc;
	private boolean useSparseRepresentation;

	public RankListIterable(String input, int chunk, boolean mustHaveRelDoc, boolean useSparseRepresentation) {
		this.input = input;
		this.chunk = chunk;
		this.mustHaveRelDoc = mustHaveRelDoc;
		this.useSparseRepresentation = useSparseRepresentation;
	}

	@Override
	public Iterator<List<RankList>> iterator(){
		return new RankListIterator(input,  chunk, mustHaveRelDoc, useSparseRepresentation);
	}
}


class RankListIterator implements Iterator<List<RankList>> {
	...
	public RankListIterator(String input, int chunk, boolean mustHaveRelDoc, boolean useSparseRepresentation) {
		try {
			this.reader = new Scanner(new FileReader(input));
		}
	...
	@Override
	public List<RankList> next() {
		ArrayList<String> lines = new ArrayList<>();
		for(int i = 0; i < chunk; i++) {
			if(!reader.hasNextLine()) break;
			lines.add(reader.nextLine());
		}
		return FeatureManager.readInput(lines, mustHaveRelDoc, useSparseRepresentation);
	}
}