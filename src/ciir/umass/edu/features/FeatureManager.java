...
public class FeatureManager {
	...
	public static List<RankList> readInput(ArrayList<String> lines, boolean mustHaveRelDoc, boolean useSparseRepresentation)
	{
		List<RankList> samples = new ArrayList<RankList>();
		int countRL = 0;
		int countEntries = 0;
		try {
			...
			for(String content: lines)
			{
				...
				if(countEntries % 10000 == 0)
					System.out.print("\rReading feature block " + countRL + "... ");
			...
			if(rl.size() > 0 && (!mustHaveRelDoc || hasRel))
				samples.add(new RankList(rl));
			System.out.println("\rReading feature block... [Done.]            ");
			System.out.println("(" + samples.size() + " ranked lists, " + countEntries + " entries read)");
		}
		catch(Exception ex)
		{
